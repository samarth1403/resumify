        ...
        if (Array.isArray(sampleData[field as keyof typeof sampleData])) {
          ...
          (
            ...
            if (field === "experience" || field === "education") {
              ...
              <div key={${index}} style="display: flex; flex-direction: column; gap:3px;">
                ...
                <p style="font-size: 0.35rem; color: gray;">${(item as experienceType).startDate || ""} - ${(item as experienceType).endDate || ""}</p>
                <p style="font-size: 0.35rem;">${(item as experienceType).jobDescription || (item as educationType).description || ""}</p>
              </div>
            `;
            } else if (field === "skills") {
              listString += `<li key={${index}} style="font-size: 0.35rem;">${item}</li>`;
            } else if (field === "profiles") {
              listString += `<li key={${index}} style="font-size: 0.35rem;"><a href=${(item as profileType).link} target="_blank" >${(item as profileType).profile}</a></li>`;
            } else if (field === "projects") {
              listString += `
              <div key={${index}} style="display: flex; flex-direction: column; gap:3px;">
                ...
                <a href=${(item as projectType).link} target="_blank" style="font-size: 0.35rem; color: ${sampleData.color};">${(item as projectType).link}</a>
              </div>
            `;
            }
          });
          jsxString =
            (
              ...
              <ul style="  ${field === "skills" || field === "profiles" ? "margin-left: 10px; list-style-type: disc;  display: flex; flex-direction: row; gap: 1rem; flex-wrap: wrap; justify-content: flex-start" : "list-style-type: disc; display: flex;flex-direction: column; gap: 0.1rem; justify-content: flex-start"}    ">
                  ${listString}
              </ul>
                 </div>`
              : "";
